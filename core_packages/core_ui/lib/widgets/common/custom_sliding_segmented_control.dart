...
const Radius _kThumbRadius = Radius.circular(10);
...
// vertex of the CustomSlidingSegmentedControl's Rect.
//
...
class _Segment<T> extends StatefulWidget {
  ...
  // Whether the thumb of the parent widget (CustomSlidingSegmentedControl)
...
///
/// {@tool dartpad}
/// This example shows a [CustomSlidingSegmentedControl] with an enum type.
///
...
class _SegmentedControlState<T> extends State<CustomSlidingSegmentedControl<T>>
  ...
  T segmentForXPosition(double dx) {
    ...
    int index = (dx ~/ (renderBox.size.width / numOfChildren)).clamp(
      0,
      numOfChildren - 1,
...
class _RenderSegmentedControl<T> extends RenderBox
  ...
  void performLayout() {
    ...
    final BoxConstraints separatorConstraints = childConstraints
        .heightConstraints();
